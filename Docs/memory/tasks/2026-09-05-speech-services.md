...
- 语音实验室改为 ASR、翻译、TTS 三张独立配置卡片；每张卡片提供模型、语言/目标语言、音色、语速或清洗参数，并展示对应模型的下载按钮与状态。
- 新增 `speech_model_manager.py` 和模型下载接口：后台线程下载、ModelScope/官方 Release 备用源、进度轮询、取消、压缩包路径校验和本地安装状态；HY-MT1.5-1.8B Q4_K_M 使用 ModelScope GGUF 镜像。
- 修复配置页语言选择被 5 秒状态轮询覆盖的问题：编辑语音配置时保留本地草稿，目标语言选择不再跳回；模型选择框现在同时显示可下载目录和本地已发现模型，并补充 Paraformer、HY-MT Q8、AISHELL3 多模型选项。
